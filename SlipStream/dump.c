...
int main() {
    ...
    for(int i=0; i < gguf->tensor_count; i++) {
        if (strstr(gguf->tensors[i].name, "attn_q.bias") || strstr(gguf->tensors[i].name, "token_embd.weight") || strstr(gguf->tensors[i].name, "output.weight") || strstr(gguf->tensors[i].name, "attn_norm.weight")) {
